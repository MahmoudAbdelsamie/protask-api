    ...
    if (index < filesnames.length) {
      if (extension != undefined && buffer != undefined) {
        if (!files_extensions.includes(extension)) {
          throw new Error("Invalid file extension format try another");
        }
        ...
        throw new Error("invalid extensions try another file format");
      }
    } else {
      throw new Error("Not enough filenames for all files");
    }
  });
  return uploadedFilesInfo;
};
